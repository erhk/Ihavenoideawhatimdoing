...
library(lme4);library(lmerTest);library(psych);library(MuMIn);library(tidyverse);library(magrittr);library(Metrics)
...
ASD_folds <- createFolds(unique(ASDdata$Child.ID, k=k))
...
for(i in 1:k){
  ...
  newlm <- lmer(CHI_MLU ~ Diagnosis * Visit + ADOS1 + (1+Visit|Child.ID), data=train) #Get your new linear model (just fit on the train data)
...
#fixef - undersøg måske
}

#ASDdata$holdoutpred #Show predictions
...
```
Report results:
We crossvaliudated our train and test data on 10 folds. We found the test error to be M = 0,70 and SE = 0,063 We found the trainingerror to be M = 0,34 and SE = 0,0022.


- Now try to find the best possible predictive model of ChildMLU, that is, the one that produces the best cross-validated results.
```{r}

#Test more models this way to find best model - loop and shit, make trainpred <- rmse(predict(new model, dataset, allow.new.levels = TRUE), dataset$CHI_MLU)
...
#MLU - 0,2 + 1*Visit + 1*Verbal IQ + 1*Diagnosis


#Bernie isnt autistic, we exclude the asd kids. We want to compare him to the average td kids. So we need the six TDs average mlu.
