...
```{r}
#convert Child.ID from factor to numeric
...
```{r}
#1. Counting, 97 children had a MLU of more than 2.7 morphemes
count(filter(ASD_data, CHI_MLU >= 2.7))

#2.Selecting Visit 1 and CHI_MLU, setting the limit of morphemes to 1.5, then count amount of
#CHI_MLU associated with Visit 1. 48 children had a MLU of less than 1.5 for their first visit.
ASD_data %>%
  dplyr::select(Visit, CHI_MLU) %>%
  filter(CHI_MLU <= 1.5 & Visit == 1) %>%
  count(Visit)

#3. Amountof children that haven't completed the 6 trials, 296
ASD_data %>%
  dplyr::select(Visit) %>%
  filter(Visit !=6) %>%
  count(Visit !=6)
...
Mean_wordtoken_All <- ASD_data %>% 
  mutate(mean(round(tokens_CHI)))
#2.
ASD_dataMean <- ASD_data %>%
  group_by(Child.ID) %>%
